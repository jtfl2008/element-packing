    ...
    render(h) {
      ...
      let renderColumn = (item, index) => {
        ...
        props = {
          props: {
            ...Object.assign(
              { key: index, 'show-overflow-tooltip': item.showTip },
              toUpperCaseKey(item)
            )
          }
        }
                      ...
                      renderEle =
                        typeof item.render == 'function'
                          ? ((retVal = item.render(
                            ...
                            retVal ? retVal : item.render)
                          : item.render
                      return renderEle.constructor.name ==
                        this.$vnode.constructor.name
                        ? renderEle
                        : h('span', renderEle)
                    } else {
                      return h(
              ...
              {...{
                scopedSlots: {
                  default: ({ ...arg }) => {
                    ...
                    return this.$scopedSlots[item.slot]({ ...arg })
                  }
                }
              }}
            />
              ...
              item.children.map((child, childIndex) => {
                if (child.children && this.showColumn(child.isShow)) {
                  return multisetTableHeader(
                    ...
                    item,
                    child,
                    renderTableColumn(child.children)
                  )
                }
